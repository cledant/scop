...
/*   Updated: 2017/03/08 18:35:41 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "scop.h"

static int		last_part(const char *str, const size_t cur_pos,
					const size_t size, const char *prev_end)
{
	extern int		errno;
	long			dummy;
	char			*begin;
					...
					const size_t size)
{
	...
	if (*end != '/' || (new_pos + 1) > size || *(end + 1) == '\0')
		return (0);
	return (last_part(str, cur_pos, size, end));
}
