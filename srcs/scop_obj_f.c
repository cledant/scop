...
/*   Updated: 2017/03/09 18:57:44 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		scop_obj_f(t_obj_read *obj, t_env *env)
{
	...
	if (expected_type == 0)
	{
		...
		if (expected_type == 1 || expected_type == 2 || expected_type == 3)
			env->group_type = GRP_V;
		else if (expected_type == 4 || expected_type == 5)
			env->group_type = GRP_V_VT;
		else if (expected_type == 6)
			env->group_type = GRP_V_VT_VN;
		else if (expected_type == 7)
			env->group_type = GRP_V_VN;
		else
			return (0);
	}
	obj->valid_state[0] = 0;
	...
	obj->valid_state[5] = 0;
	obj->valid_state[6] = 1;
	return (scop_get_faces(obj, env, expected_type, env->group_type));
}
