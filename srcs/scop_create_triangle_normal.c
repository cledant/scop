...
/*   Updated: 2017/03/10 11:00:41 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		scop_create_triangle_normal(t_env *env, const size_t index)
{
	...
	scop_vec3_set(&ab, env->obj.cvrt[index + 1].pos.x -
	...
	scop_vec3_set(&ac, env->obj.cvrt[index + 2].pos.x -
		env->obj.cvrt[index].pos.x, env->obj.cvrt[index + 2].pos.y -
		env->obj.cvrt[index].pos.y, env->obj.cvrt[index + 2].pos.z -
		env->obj.cvrt[index].pos.z);
	scop_vec3_cross_prod(&norm, ab, ac);
	scop_vec3_set(&(env->obj.cvrt[index].norm), norm.x, norm.y, norm.z);
	scop_vec3_set(&(env->obj.cvrt[index + 1].norm), norm.x, norm.y, norm.z);
	scop_vec3_set(&(env->obj.cvrt[index + 2].norm), norm.x, norm.y, norm.z);
	return (1);
}
